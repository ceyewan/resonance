      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-1.5 custom-scrollbar relative z-10">
        {filteredSessions.length === 0 ? (
          <div className="text-center mt-10 space-y-3 px-4">
            <p className="text-[15px] text-[var(--color-text-muted)] opacity-70">No messages yet.</p>
            <Link to="/contacts" className="inline-flex rounded-full border border-[var(--color-border)] bg-[var(--glass-surface)] px-4 py-2 text-sm text-[var(--color-text)] hover:bg-[var(--glass-surface-hover)] transition-colors">
              Go to Contacts
            </Link>
          </div>
        ) : (
                ...
                {({ isActive }) => (
                  <>
                    <div className="flex justify-between items-center mb-1 gap-2">
                      <span className={`font-semibold truncate text-[16px] ${isActive ? 'text-[var(--color-text)]' : 'text-[var(--color-text)] group-hover:text-[var(--color-text)]'}`}>
                        {session.name || session.sessionId}
                      </span>
                      {Number(session.unreadCount) > 0 ? (
                        <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full shrink-0 shadow-sm ${isActive ? 'bg-[var(--glass-surface)] text-[var(--color-primary)]' : 'bg-[var(--color-primary)] text-[var(--color-text)]'}`}>
                          {session.unreadCount}
                        </span>
                      ) : null}
                    </div>
                    <p className={`text-[14px] truncate ${isActive ? 'text-[var(--color-text)] opacity-90' : 'text-[var(--color-text-muted)] opacity-70 group-hover:text-[var(--color-text-muted)]'}`}>
