...
func New(cfg *Config) (*Gateway, error) {
	...
	// 初始化 Logic 客户端
	logicClient, err := NewLogicClient(cfg.LogicAddr, cfg.GatewayID, logger)
	if err != nil {
...
func (g *Gateway) Run() error {
	...
	// 启动 HTTP 服务（RESTful API + Push RPC）
...
func (g *Gateway) startHTTPServer() {
	...
	// 注册 Push Service（ConnectRPC）
	pushPath, pushHandler := g.pushService.GetHandler()
	router.Any(pushPath+"*any", gin.WrapH(pushHandler))
